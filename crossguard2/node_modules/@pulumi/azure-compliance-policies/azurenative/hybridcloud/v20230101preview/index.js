"use strict";
// Copyright 2016-2024, Pulumi Corporation.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
//
// ------------------------------- WARNING -------------------------------------
// This file was programmatically generated. Do not edit unless you know what
// you're doing.
// ------------------------------- WARNING -------------------------------------
Object.defineProperty(exports, "__esModule", { value: true });
exports.CloudConnector = exports.CloudConnection = void 0;
const policy_1 = require("@pulumi/policy");
const compliance_policy_manager_1 = require("@pulumi/compliance-policy-manager");
const v20230101preview_1 = require("@pulumi/azure-native/hybridcloud/v20230101preview");
const v20230101preview_2 = require("@pulumi/azure-native/hybridcloud/v20230101preview");
var CloudConnection;
(function (CloudConnection) {
    /**
     * Disallow the use of non-stable (Preview) Azure resouces (hybridcloud.v20230101preview.CloudConnection).
     *
     * @severity medium
     * @frameworks none
     * @topics api, preview, unstable
     * @link https://learn.microsoft.com/en-us/rest/api/azure/
     */
    CloudConnection.disallowPreviewResource = compliance_policy_manager_1.policyManager.registerPolicy({
        resourceValidationPolicy: {
            name: "azurenative-hybridcloud-v20230101preview-cloudconnection-disallow-preview-resource",
            description: "Disallow the use of non-stable (Preview) Azure resouces (hybridcloud.v20230101preview.CloudConnection).",
            configSchema: compliance_policy_manager_1.policyManager.policyConfigSchema,
            enforcementLevel: "advisory",
            validateResource: (0, policy_1.validateResourceOfType)(v20230101preview_1.CloudConnection, (_, args, reportViolation) => {
                if (!compliance_policy_manager_1.policyManager.shouldEvalPolicy(args)) {
                    return;
                }
                reportViolation("Azure CloudConnection shouldn't use an unstable API (hybridcloud.v20230101preview.CloudConnection). A compatible replacement can be found at 'hybridcloud.CloudConnection'.");
            }),
        },
        vendors: ["azure"],
        services: ["hybridcloud"],
        severity: "medium",
        topics: ["api", "unstable", "preview"],
    });
})(CloudConnection || (CloudConnection = {}));
exports.CloudConnection = CloudConnection;
var CloudConnector;
(function (CloudConnector) {
    /**
     * Disallow the use of non-stable (Preview) Azure resouces (hybridcloud.v20230101preview.CloudConnector).
     *
     * @severity medium
     * @frameworks none
     * @topics api, preview, unstable
     * @link https://learn.microsoft.com/en-us/rest/api/azure/
     */
    CloudConnector.disallowPreviewResource = compliance_policy_manager_1.policyManager.registerPolicy({
        resourceValidationPolicy: {
            name: "azurenative-hybridcloud-v20230101preview-cloudconnector-disallow-preview-resource",
            description: "Disallow the use of non-stable (Preview) Azure resouces (hybridcloud.v20230101preview.CloudConnector).",
            configSchema: compliance_policy_manager_1.policyManager.policyConfigSchema,
            enforcementLevel: "advisory",
            validateResource: (0, policy_1.validateResourceOfType)(v20230101preview_2.CloudConnector, (_, args, reportViolation) => {
                if (!compliance_policy_manager_1.policyManager.shouldEvalPolicy(args)) {
                    return;
                }
                reportViolation("Azure CloudConnector shouldn't use an unstable API (hybridcloud.v20230101preview.CloudConnector). A compatible replacement can be found at 'hybridcloud.CloudConnector'.");
            }),
        },
        vendors: ["azure"],
        services: ["hybridcloud"],
        severity: "medium",
        topics: ["api", "unstable", "preview"],
    });
})(CloudConnector || (CloudConnector = {}));
exports.CloudConnector = CloudConnector;
